...
export default function SelectInput({ choices, selectedChoice, setSelectedChoice }: SelectInputProps) {
  return (
    <Listbox value={selectedChoice} onChange={setSelectedChoice}>
      <div className="relative w-full mt-1">
        ...
        <Transition
          ...
          leaveTo="opacity-0"
        >
          <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white bg-opacity-10 py-1 text-base shadow-lg ring-1 ring-white ring-opacity-5 focus:outline-none sm:text-sm">
            {choices.map((choice, index) => (
              <Listbox.Option
                key={index}
                className={({ active }) =>
                  `relative cursor-default select-none py-2 pl-10 pr-4 ${
                    active ? "bg-white bg-opacity-30 text-bold" : "text-white"
                  }`
                }
                value={choice}
              >
                {({ selected }) => (
                  <>
                    ...
                    {selected ? (
                      <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-white text-bold">
