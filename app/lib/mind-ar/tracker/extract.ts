 ...
 */
export const extract = (image: ImageParams) => {
...
type SelectFeatureOptions = { image: ImageParams, featureMap: Float32Array, templateSize: number, searchSize: number, occSize: number, maxSimThresh: number, minSimThresh: number, sdThresh: number, imageDataCumsum: Cumsum, imageDataSqrCumsum: Cumsum }

const _selectFeature = (options: SelectFeatureOptions) => {
...
const _templateVar = ({ image, cx, cy, sdThresh, imageDataCumsum, imageDataSqrCumsum }: TemplateParams) => {
...
type SimilarityOptionParams = {
  image: ImageParams, cx: number, cy: number, vlen: number, tx: number, ty: number, imageDataCumsum: Cumsum, imageDataSqrCumsum: Cumsum,
}

const _getSimilarity = (options: SimilarityOptionParams) => {
